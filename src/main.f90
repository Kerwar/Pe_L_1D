program main
  ...
  allocate(A(2,3*n), b(2,3*n), x(2,3*n), xp(2,3*n), &
  ...
  do i = 1,maxiter  
    ...
    call temperature(A(:,1:n), b(:,1:n), h, Pe, bK, q, rho, &
    x(:,2*n+1:3*n), x(:,1:n), nmin, nmax)
    CALL onestepgaussseidel(A(1,1:n), b(1,1:n), x(1,1:n), xp(1,1:n))
    CALL onestepgaussseidel(A(2,1:n), b(2,1:n), x(2,1:n), xp(2,1:n))
    ...
    CALL massfractionF(A(:,n+1:2*n), b(:,n+1:2*n) , h, Pe, Le_f, &
    beta, gamma, rho, x(:,2*n+1:3*n), x(:,1:n))
    CALL onestepgaussseidel(A(1,n+1:2*n), b(1,n+1:2*n), &
    x(1,n+1:2*n), xp(1,n+1:2*n))
    CALL onestepgaussseidel(A(2,n+1:2*n), b(2,n+1:2*n), &
    x(2,n+1:2*n), xp(2,n+1:2*n))
    x(:,n+1:2*n) = (1d0-relax)*xp(:,n+1:2*n) + relax*x(:,n+1:2*n)
    
    CALL massfractionZ(A(:,2*n+1:3*n), h, Pe, Le_z, beta, gamma, rho, x(:,n+1:2*n), x(:,1:n))
    CALL onestepgaussseidel(A(1,2*n+1:3*n), b(1,2*n+1:3*n), x(1,2*n+1:3*n), xp(1,2*n+1:3*n))
    CALL onestepgaussseidel(A(2,2*n+1:3*n), b(2,2*n+1:3*n), x(2,2*n+1:3*n), xp(2,2*n+1:3*n))
